...
{   //NumPhysPages
    ...
    pageTable = new TranslationEntry[NumPhysPages]; //加入了virtual的概念(pagetable即是virtual memory)
...
// insert start
	...
	for(unsigned int i = 0, j = 0; i < numPages; i++) {
		...
		pageTable[i].physicalPage = j; //確保load一個page
		pageTable[i].valid = true;
		pageTable[i].use = false;
		pageTable[i].dirty = false;
		pageTable[i].readOnly = false;
...
// Note: this code assumes that virtual address = physical address
    int pn = noffH.code.virtualAddr/PageSize;
	...
	DEBUG(dbgAddr, noffH.code.virtualAddr << ", " << noffH.code.size);
        int load = std::min(codesize,PageSize);
        executable->ReadAt(
        &(kernel->machine->mainMemory[pageTable[pn].physicalPage
        * PageSize ]), 
        load, addr);
        }
        pn++;
