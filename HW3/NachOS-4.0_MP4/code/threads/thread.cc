...
Thread::Yield ()
{
    ...
    double oldPBurstTime = this->ApproximateBurst; //ti-1
    double T = this->getBurstTime(); //T
    
    this->Remaining = std::max(0.0,oldPBurstTime-T);
    //cout<<"Thread "<<this->getID()<<" Set Remaining : "<<this->Remaining<<endl;
    ...
    " is now selected for execution, thread ["<<this->getID()<<"] is preempted"<<
    ", and it has executed ["<<this->getBurstTime()<<"] ticks");

    if(kernel->currentThread->Remaining>nextThread->Remaining){ 
...
Thread::Sleep (bool finishing)
{
    ...
    status = BLOCKED;
    this->end = kernel->stats->totalTicks;
    this->setBurstTime((this->end-this->start)+this->getBurstTime());   //accumulate
   

    double BurstTime = this->getBurstTime();   //T
    double PreApproximateBurst = this->ApproximateBurst; //ti-1
    this->ApproximateBurst = 0.5*BurstTime + 0.5*PreApproximateBurst; //ti //更新

    //[C]
    DEBUG(dbgSche,"[C] Tick ["<<kernel->stats->totalTicks<<"]: Thread "<<kernel->currentThread->getID()<<
    " update approximate burst time, from: "<<0.5*PreApproximateBurst<<" add "<<0.5*BurstTime<<
    " , to "<<this->ApproximateBurst);

    this->setBurstTime(0);  //Burst Time = 0
    ...
    " is now selected for execution, thread ["<<this->getID()<<"] starts IO"<<
    ", and it has executed ["<<BurstTime<<"] ticks");
